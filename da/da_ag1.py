...
def generarEvento(sRutaEntrada:str, sRutaSalida:str, lEvento:list, fPorcentaje:float, iSegmentoCruce:int, bAjusteSegmentoContiguo:bool, iCantidad:int):
  ...
  Args:
      ...
      sRutaSalida (str): Folder where generated events will be saved
      lEvento (list): Events to consider for generation
      fPorcentaje (float): Percentage of signal in child from first parent (1-fPorcentaje from second)
      iSegmentoCruce (int): Number of segments for crossover
      bAjusteSegmentoContiguo (bool): If True, merge contiguous segments into one
      iCantidad (int): Number of events to generate
  """
  # Reading events
  for sEvento in lEvento:
